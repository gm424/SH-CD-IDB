			...
			<view class="content">
				<view class="info">
					...
					<view class="right t-w" :class="{ useImgSku: isUseImgSku }">
						<view class="price">
							<text class="uity" :style="{ color: themeRGB }">{{ this.currency_tpyes }}</text>
						...
						<text class="stock" v-if="showStockNum">
							{{ $t('Inventory') }}: {{ selectSku.id ? selectSku.stock : `${showAreaStock[0]}-${showAreaStock[1]}` }}
						</text>
...
export default {
	...
	methods: {
		...
		init(data) {
			...
			// 获取币种
			this.currency_tpyes = uni.getStorageSync('currency_tpyes');
	...
	computed: {
		...
		getSelectedSkuAttrStr() {
			...
			if (attrArr.findIndex(item => !item) !== -1) {
				...
				return this.$t('Please select') + `：${noAttrNameArr.join('、')}`
			} else {
				return this.$t('Post an inquiry') + `：${attrArr.join('，')}`
			}
